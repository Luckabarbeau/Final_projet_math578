...
for i=1:nbpoint_immersed
    ...
    cell=which_cell(mesh,X(i,:));
    if isempty(find(cells_immerged==cell))
        cells_immerged=[cells_immerged;cell];
    end
    if isempty(find(mesh.immersed_iteration(neighbors(mesh,cell,1))==3  ))
        nb_lagrange_max=2;
    else
        nb_lagrange_max=2;
    end
    if mesh.immersed_iteration(cell)<nb_lagrange_max
        mesh.immersed_iteration(cell)=mesh.immersed_iteration(cell)+1;
        a_p=get_lagrange_multiplier_stancil_weak_nearest(mesh,cell,X(i,:),n_dof_domain);
        C_Sparse_index=[C_Sparse_index ;sparsematrix(a_p,k+n_dof_domain)];
        boundary_value=immersed_boundary_value_function(X(i,:));
        B_immersed=[B_immersed ; boundary_value];
        B_immersed_value=[B_immersed_value ; boundary_value];
        immersed_used=[immersed_used ; X(i,:)] ;
        k=k+1;
    end
...
hold on
plot3(immersed_used(:,1),immersed_used(:,2),B_immersed_value,'.r','markersize',30);
 pause(0.001);
hold off
...
if(j<nb_refinement)
mesh=raffine(mesh,Solution,1,0,cells_immerged);
mesh=update_neighbors_v(mesh);
end
