...
%         a_p(1,i+length(mesh.points)*2)=1; %speed value Y
%         k=k+1;
%         E_Sparse_index=[E_Sparse_index ; sparsematrix(a_p,l+k+2*length(cells_sum.boundary)+length(mesh.points))];
      end
...
Constraint_sparse_matrix=sparse(Constraint_sparse_matrix_index(:,1),Constraint_sparse_matrix_index(:,2),Constraint_sparse_matrix_index(:,3),l+k+2*length(cells_sum.boundary),2*length(mesh.points));

Constraint_sparse_conservation=sparse(C_Sparse_index(:,1),C_Sparse_index(:,2),C_Sparse_index(:,3));
Constraint_sparse_pressure=sparse(P_Sparse_index(:,1),P_Sparse_index(:,2),P_Sparse_index(:,3));

constraint_total=[Constraint_sparse_conservation;Constraint_sparse_matrix];
constraint_total_T=[Constraint_sparse_pressure Constraint_sparse_matrix'];


A_Sparse_global=[A_sparse_matrix constraint_total_T ; -constraint_total*A_sparse_matrix -constraint_total*constraint_total_T];
...
% P_sparse_m=sparse(P_Sparse_index(:,1),P_Sparse_index(:,2),P_Sparse_index(:,3));
...
% 
% [F_v_x, F_v_y]=force_on_boundary_viscosity(mesh,dux_dy,duy_dx,mu,X2);
...
% 
 [Solution max_du_dt]=Runge_kutta_NS(Solution,t,dt,A_Sparse_global,mesh,Boundary);
% step_dudt=(back_euler_M\Solution);
% 
%  step_dudt=clear_dudt_boundary_NS(step_dudt,Boundary,mesh);
% Solution=Solution+step_dudt;

% [Solution max_du_dt]=Runge_kutta_NS_backward(Solution,t,dt,back_euler_M);
...
A_Sparse_global=[A_momentum constraint_total_T ; -constraint_total*A_momentum -constraint_total*constraint_total_T];
