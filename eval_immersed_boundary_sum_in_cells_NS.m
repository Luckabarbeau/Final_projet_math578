function cells_boundary_sum= eval_immersed_boundary_sum_in_cells1(cells,X,type)
...
for i=1:length(cells_boundary_sum.boundary)
        for j=1:length(cells_boundary_sum.boundary{i})
            cells_boundary_sum.sum(i,1)=cells_boundary_sum.sum(i,1)+immersed_boundary_value_function_NS(X(cells_boundary_sum.boundary{i}(j),:),type);
        end
end
else
